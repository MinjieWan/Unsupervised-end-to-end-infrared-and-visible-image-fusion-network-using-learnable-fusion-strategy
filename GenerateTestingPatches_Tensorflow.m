...
for i = 1 : length(subfolder)
    ...
    for s = 1:4
        ...
        for x = 1+step : stride : (hei-patchsize1+1)
            for y = 1+step :stride : (wid-patchsize2+1)
                count = count+1;
            end
        end
    end
...
inputs  = zeros(numPatches,patchsize1, patchsize2, 2, 'single'); % this is fast
...
count = 0;
for i = 1 : length(subfolder)
        S0 =double(imread(fullfile(folder,subfolder{i},imagename{1}))); % VIS
        DoLP = double(imread(fullfile(folder,subfolder{i},imagename{2}))); % IR
        if size(S0,3)==3
            S0 = rgb2gray(S0);
        end
        if size(DoLP,3)==3
...
%         DoLP = sqrt(S1.^2+S2.^2)./S0;
        for s = 1:4
            ...
            for x = 1+step : stride : (hei-patchsize1+1)
                for y = 1+step :stride : (wid-patchsize2+1)
                    count       = count+1
                    inputs(count,:, :, 1)   = im_s0(x : x+patchsize1-1, y : y+patchsize2-1,:);
                    inputs(count,:, :, 2)   = im_dolp(x : x+patchsize1-1, y : y+patchsize2-1,:);
                end
            end
        end
end
...
%%% save data
save(fullfile(dataName,['imdb_',num2str(patchsize1),'_',num2str(batchSize)]), 'inputs','-v7.3')
